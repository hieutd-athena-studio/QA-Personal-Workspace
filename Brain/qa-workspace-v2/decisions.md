...
- **2026-05-18 — happy-dom over jsdom for Vitest** — jsdom 29 transitive dep chain breaks under Node 20 (html-encoding-sniffer → @exodus/bytes/encoding-lite.js, ESM-only required via CJS). happy-dom is lighter (faster CI) and ESM-friendly. Alt rejected: stay on jsdom and add `"type": "module"` to package.json (risks Electron main process CJS build).
- **2026-05-18 — Auto-generate routeTree.gen.ts via `tsr generate` pre-typecheck** — File is gitignored; regenerated from `src/renderer/src/routes/` on every typecheck and build. Alt rejected: commit the generated file (rot risk if developer forgets to regen after route change).
- **2026-05-18 — shadcn UI files get blanket eslint-disable header** — shadcn pattern co-exports `buttonVariants` (cva) alongside `Button` component, which trips react-refresh/only-export-components; shadcn also omits explicit return types. Disabling 2 rules per file beats restructuring upstream-owned code. Alt rejected: extract variants to separate files (fights the shadcn upgrade path) or weaken global eslint config (config-protection hook blocks this anyway).
- **2026-05-18 — Custom TS migration runner over drizzle-kit-generated SQL** — Hand-rolled runner gives us atomic BEGIN/COMMIT + snapshot-before-run hook + typed MigrationError + idempotence check via meta.schema_version, none of which drizzle-kit's stock migrator provides out of the box. Alt rejected: use drizzle-kit's `migrate()` (no built-in snapshot, harder to inject backup step).
- **2026-05-18 — Hash history for TanStack Router under Electron** — `createHashHistory()` works against `file://` URLs in production builds; pushState would 404. Alt rejected: memory history (loses deep-link/refresh ability), browser history (breaks under file:// protocol).
